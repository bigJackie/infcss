import type { ParsedResult, Rule } from '@infcss/shared'
import { posContentMap } from '../themes/index.ts'
...
export const position: Rule[] = [
  ...
  [
    ...
    ([, val]) => ({
      ...
      '-ms-flex-align': val,
      '-webkit-align-items':
        val === 'start' || val === 'end' ? `flex-${val}` : val,
      'align-items': val === 'start' || val === 'end' ? `flex-${val}` : val,
    }),
  ],

  // align-self (auto|center|stretch)
  [
  ...
  [
    ...
    ([, val]) => ({
      'place-content': ['between', 'around', 'evenly'].includes(val)
        ? `space-${val}`
        : val,
    }),
  ],
  ...
  [
    ...
    ([, val]) => ({
      position: val === 'sticky' ? [`-webkit-${val}`, val] : val,
    }),
  ],
